...
SELECT p.Name AS 'Product Name', SUM(s.Quantity) AS 'Total Quantity', SUM(s.Quantity * s.PricePerUnit) AS 'Total Price'
FROM products AS p
...
SELECT e.EmployeeID, e.FirstName, e.LastName, p.Name AS 'Product Name', Sum(s.Quantity) AS 'Total Quantity'
FROM sales AS s
INNER JOIN employees AS e ON e.EmployeeID = s.EmployeeID
